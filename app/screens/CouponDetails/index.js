...
const CouponDetails = (props) => {
    ...
    const getCouponProducts = () => {
        ...
        var filter = {
            ...
            "Partner_Details_Id": coupon?.Partner_Details_Id,
        }
        setPartnerProducts([]);
    ...
    const copyCode = () => {
        ...
        global.Coupon_Id = coupon?.Coupon_Id;
        global.Coupon_Code = coupon?.Code;
        Toast.show("The coupon code has been copied.", Toast.LONG);
    }
        ...
        >
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                ...
                >
                    ...
                    <Text style={{ fontSize: 20, fontWeight: 'bold', color: 'white' }}>Coupon ({coupon?.Name})</Text>
                </View>
            </View>
            ...
            {loading ? <BallIndicator color={EStyleSheet.value('$primaryColor')} />
                ...
                <ScrollView style={{ flex: 1, padding: 10 }} showsVerticalScrollIndicator={false}>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <Image source={{ uri: coupon?.Image_Path }} style={{ width: 100, height: 100 }} />
                        <Text style={{ fontSize: 18, fontWeight: 'bold', flex: 1, textAlign: 'center', color: EStyleSheet.value('$primaryColor') }}>{coupon?.Name}</Text>
                    </View>
                    <HTMLView value={coupon?.Description} />
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <Text style={{ flex: 1, color: 'black', fontSize: 16, fontWeight: 'bold' }}>Status:   {coupon?.Expired_Status}</Text>
                    </View>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <Text style={{ fontSize: 16, flex: 1, color: EStyleSheet.value('$grayColor') }}>ESI Cashback</Text>
                        <Text style={{ flex: 2, color: 'black' }}>{coupon?.ESI_Cashback_Amount} %</Text>
                    </View>
                    <View style={{ flexDirection: 'row' }}>
                        <Text style={{ fontSize: 14, textAlign: 'center', flex: 1 }}>Valid  from  {coupon?.Valid_From_Date}  to  {coupon?.Valid_Through_Date}</Text>
                    </View>
                    <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 10 }}>
                        <Text style={{ fontSize: 16 }}>Coupon Code:   {coupon?.Code}</Text>
                        <TouchableOpacity onPress={() => { copyCode() }} style={{ marginLeft: 20 }}>
                            <Text style={{ textAlign: 'center', backgroundColor: EStyleSheet.value('$mainColor1'), color: 'white', borderRadius: 10, paddingHorizontal: 20 }}>Copy</Text>
                        </TouchableOpacity>
                    </View>
